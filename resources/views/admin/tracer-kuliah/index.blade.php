    ...
    <div class="container mx-auto mt-8">
        <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <div
                ...
                <h1 class="text-2xl font-bold">Daftar Tracer Kuliah</h1>
                <a href="{{ route('admin.tracer-kuliah.create') }}"
                    class="bg-white text-blue-600 font-semibold py-2 px-5 rounded-lg shadow hover:bg-blue-100 transition duration-300">
                    Tambah Data Tracer Kuliah
                </a>
            </div>
                ...
                <table class="min-w-full border border-gray-300 rounded-lg overflow-hidden">
                    <thead class="bg-gray-100 border-b border-gray-300">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700">Kampus</th>
                            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700">Status</th>
                            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700">Jenjang</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($tracers as $tracer)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800 font-medium">{{ $tracer->tracer_kuliah_kampus }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-800 font-medium">{{ $tracer->tracer_kuliah_status }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-800 font-medium">{{ $tracer->tracer_kuliah_jenjang }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex space-x-4">
                                        <!-- Edit Button -->
                                        <a href="{{ route('admin.tracer-kuliah.edit', $tracer->id_tracer_kuliah) }}"
                                        ...
                                        <!-- Delete Button -->
                                        <form action="{{ route('admin.tracer-kuliah.destroy', $tracer->id_tracer_kuliah) }}"
                                            method="POST" class="inline"
                                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus tracer kuliah ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
